function main()
   ...
   cHtml += "        <select id='voice-select' style='background: var(--glass); color: white; border: 1px solid var(--glass-border); border-radius: 5px; padding: 0.2rem; outline: none; display: none; max-width: 150px;'></select>"
   cHtml += "        <button id='tts-btn' onclick='toggleTTS()' style='background:transparent; padding:0; font-size:1.5rem; outline:none; cursor:pointer;' title='Voz del Bot'>🔊</button>"
   ...
   cHtml += "        <button id='mic-btn' onclick='toggleSpeech()' title='Hablar'>🎤</button>"
   ...
   cHtml += "        if (ttsEnabled) { btn.innerHTML = '🔊'; btn.style.opacity = '1'; sel.style.opacity = '1'; } else { btn.innerHTML = '🔇'; btn.style.opacity = '0.5'; sel.style.opacity = '0.5'; if (window.speechSynthesis) window.speechSynthesis.cancel(); }"
   cHtml += "    }"
   cHtml += "    function speakText(text) {"
   cHtml += "        if (!ttsEnabled || !window.speechSynthesis) return;"
   cHtml += "        window.speechSynthesis.cancel();"
   cHtml += "        var cleanText = text.replace(/```[\\s\\S]*?```/g, 'bloque de código omitido').replace(/[*_#`]/g, '').replace(/<[^>]+>/g, '');"
   ...
   cHtml += "            if (event.error === 'not-allowed') alert('Permiso de micrófono denegado. Revisa la configuración de permisos.');"
   cHtml += "            stopListening();"
   cHtml += "        };"
