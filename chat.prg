function main()
   ...
   cHtml += "        .sidebar-item { background: rgba(0,0,0,0.3); border-radius: 8px; padding: 1rem; margin-bottom: 0.6rem; transition: transform 0.2s, background 0.2s; }"
   cHtml += "        .sidebar-item:hover { transform: translateX(5px); background: rgba(0,0,0,0.5); }"
   ...
   cHtml += "        <button id='hamburger-btn' onclick='toggleSidebar()'>☰</button>"
   cHtml += "        <h1>AIOS</h1>"
   cHtml += "    </div>"
   ...
   cHtml += "           <button id='attach-btn' onclick='document.getElementById(" + Chr(34) + "file-upload" + Chr(34) + ").click()' title='Adjuntar archivo' style='background: transparent; padding: 0.5rem; font-size: 1.4rem; border:none; outline:none;'>📎</button>"
